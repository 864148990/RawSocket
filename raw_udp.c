...
int main(int argc, char const *argv[])
{
    ...
    *(uint16_t *)(buf) = htons(0x0800); // ipv4
    ...
    udp->len = htons(pkt_len - sizeof(struct iphdr));
    udp->check = udp_checksum(udp, pkt_len - sizeof(struct iphdr), src_addr, dst_addr);
    ...
    int32_t len = sendto(sd, (void *)data, PACKET_LEN, 0, (const struct sockaddr *)&sockaddr, sizeof(sockaddr));
    if (len > 0)
    {
